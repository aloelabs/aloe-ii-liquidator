      ...
      factory.watchEvent.CreateBorrower(
        { pool: undefined },
        {
          ...
          async onLogs(logs) {
            ...
            borrowers.push(...Array.from(new Set(logs.map((log) => log.args.account!)).values()));
          },
        }
      )
    );
        ...
        onBlockNumber(blockNumber) {
          ...
          borrowers.forEach(async (borrower) => {
            try {
              ...
              setTimeout(() => {
                console.log(`Would liquidate borrower with address: ${borrower}`);
              }, Number(timeUntilLiquidation));
            } catch (e) {
              console.error(e);
            }
          });
...
export async function getBorrowers(
  factory: GetContractReturnType<
    ...
    PublicClient,
    `0x${string}`,
    ExtractAbiEventNames<typeof factoryAbi>
  >,
  blockNumber: bigint
): Promise<Address[]> {
  let borrowers: Address[] = [];
  const createBorrowerEvents = await factory.getEvents.CreateBorrower(
    { pool: undefined },
    { strict: true, fromBlock: blockNumber }
  );
