...
const GAS_INCREASE_FACTOR: number = 1.1;
...
export default class TXManager {
  ...
  public async processLiquidatableCandidates() {
    ...
    for (let i = 0; i < this.queue.length; i++) {
      ...
      if (this.isLiquidationInProgress(borrower)) continue;
      ...
      const encodedData = this.liquidatorContract.methods
        .liquidate(borrower, encodedAddress, liquidationTxInfo.currentStrain)
        .encodeABI();
        ...
        .on('receipt', async (receipt) => {
          if (receipt.status) {
            if (this.baseEtherscanURL !== null) {
              log(
                'info',
                `💦 Borrower \`${borrower}\` has been liquidated! ${this.baseEtherscanURL}${receipt.transactionHash}`
              );
            } else {
            ...
            switch (
              reason // Used a switch b/c there might be custom logic for other revert codes
            ) {
              case 'Aloe: healthy':
